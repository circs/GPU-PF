...
#define GRADIENT (12. * 100.) // K/m
#define VELOCITY (6.)         // microns/s
#define TIMESEC (500.)

// NOUTFIELDS controls the number of for the phase field outputs; NSVGOUT controls the number of preconditioned phase field outputs
...
//		SIMULATION
// -----------------------
#define E 110.4 // E=W/d0

#define Nx 2518 // Dimension x
#define Ny 418  // Dimension y

#define dx 1.2    // Space step
#define dt0 5.e-4 // Time step (input, may be reajusted)
...
//		  OUTPUT
// -----------------------
#define PREFIX "DSI_R_PRISMS_" // Prefix of Output File Name
...
void WriteFields(char Prefix[LENMAX], int index, REAL *P, REAL *F, REAL *Cu, int *Pha, Constants *Param, int SVG);
...
int main(int argc, char **argv)
{
    ...
    // ------------------------------------------------------
    // ----------------- Physical parameters ----------------
    REAL m = 1.365;    // |Liquidus slope|
    REAL c0 = 0.46;    // Nominal composition
    REAL kcoeff = 0.1; // Partition coefficient, K
    REAL Diff = 270.;  // Diffusion, microns^2/s
    REAL Eps4 = 0.011;
    REAL Gamma = 6.4e-8;
    // ------------------ Process parameters -----------------
    ...
    REAL iq = 6.;   // Wavenumber of the initial perturbation
    REAL amp = 10.; // Amplitude of the initial perturbation
    // ----------------- Computational ----------------------
    ...
    (*h_Parameters).IterOutSvg = IterOutSvg;
...
// ------------- Initializations -------------
...
#endif
#if (NOUTSVG > 0)
    printf(" SVG Output every %d iterations\n", IterOutSvg);
#endif
    ...
    // Write preconditioned initial fields anyway
    WriteFields(OutputPrefix, 0, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 1);

// Write original phase field is NOUTFIELDS is none zero
#if (NOUTFIELDS > 0)
    WriteFields(OutputPrefix, 0, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 0);
#endif
...
////////////////////////
        ...
        if (iter % IterOutFields == 0 && iter < niter)
        {
            ...
            WriteFields(OutputPrefix, indx, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 0);
            printf("written.\n");
        }
#endif
// ===============
...
//////////////////////
// Output SVG(preconditioned) data
//////////////////////
#if (NOUTSVG > 0)
        if (iter % IterOutSvg == 0 && iter < niter)
        {
            cudaMemcpy(h_U, Ucurr, SizeGrid * sizeof(REAL), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_Psi, Psicurr, SizeGrid * sizeof(REAL), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_Phase, Phase, SizeGrid * sizeof(int), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_Parameters, Parameters, sizeof(Constants), cudaMemcpyDeviceToHost);
            int indx = (iter + 1) / IterOutSvg;
            printf("Writing file ***_%s.%d.*** ... ", OutputPrefix, indx);
            WriteFields(OutputPrefix, indx, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 1);
            printf("written.\n");
        }
#endif
    ...
    // Output preconditioned fields by default
    WriteFields(OutputPrefix, IndexFINAL, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 1);
#if (NOUTFIELDS > 0)
    WriteFields(OutputPrefix, IndexFINAL, h_Psi, h_Phi, h_U, h_Phase, h_Parameters, 0);
#endif
    printf("written.\n");
...
#define IterOutSvg (*Param).IterOutSvg
...
__global__ void Init(REAL *P1, REAL *P2, REAL *Cu1, REAL *Cu2, int *Pha, Constants *Param)
{
    ...
    REAL xinty = xi / dx + 0.5 * amp * (1. + cos(iq * 3.14159 * (Ny - j + 0.5) / Ny));
    ...
    REAL c = (i < xinty) ? (0.5 * (opk - omk * phi)) * (1. - xinit * omk) : (0.5 * (opk - omk * phi)) * (kcoeff + omk * (1. - xinit) * exp(-(i - xinty) * dx * Vp / D));
...
__global__ void Compute_P(REAL *Pcurr, REAL *Pnext, REAL *F, REAL *Cu, int *Pha, Constants *Param, curandState *state)
{
    ...
    if (i != 0 && i != Nx + 1 && j != 0 && j != Ny + 1)
    {
        ...
        if (fabs(omp2) >= 1.e-6 && fabs(NormG2) > 1.e-15)
        {
            if (!pha && fabs(omp2) >= .01)
            {
                int sum = Pha[pos(i + 1, j)] + Pha[pos(i - 1, j)] + Pha[pos(i, j + 1)] + Pha[pos(i, j - 1)] + Pha[pos(i + 1, j + 1)] + Pha[pos(i + 1, j - 1)] + Pha[pos(i - 1, j + 1)] + Pha[pos(i - 1, j - 1)];
                if (sum)
        ...
        Pnext[pos(i, j)] += dt * (phxx + phyy                                      // the remaining 2nd term when computing "anis"
                                  ...
                                  + Eps4 * anis) /
                            ((1. + Eps4 * c4) * (1. + Eps4 * c4) * Tau); // 1st + 2nd(incomplete) + 3rd terms
...
#define djyd dpyd
    ...
    if (i != 0 && i != Nx + 1 && j != 0 && j != Ny + 1)
    {
        ...
        if (fabs(omp2) >= 1.e-6)
        {
            ...
            if ((dnormr * dnorml * dnormu * dnormd) > 1.e-15)
            {
                djxr = 0.5 * ((1. - F[pos(i + 1, j)] * F[pos(i + 1, j)]) / sqrt2 * (Pnext[pos(i + 1, j)] - Pcurr[pos(i + 1, j)]) / dt * (1. + omk * Cucurr[pos(i + 1, j)]) + dphidt * (1. + omk * u)) * dpxr / dnormr;
                djxl = 0.5 * ((1. - F[pos(i - 1, j)] * F[pos(i - 1, j)]) / sqrt2 * (Pnext[pos(i - 1, j)] - Pcurr[pos(i - 1, j)]) / dt * (1. + omk * Cucurr[pos(i - 1, j)]) + dphidt * (1. + omk * u)) * dpxl / dnorml;
                djyu = 0.5 * ((1. - F[pos(i, j + 1)] * F[pos(i, j + 1)]) / sqrt2 * (Pnext[pos(i, j + 1)] - Pcurr[pos(i, j + 1)]) / dt * (1. + omk * Cucurr[pos(i, j + 1)]) + dphidt * (1. + omk * u)) * dpyu / dnormu;
                djyd = 0.5 * ((1. - F[pos(i, j - 1)] * F[pos(i, j - 1)]) / sqrt2 * (Pnext[pos(i, j - 1)] - Pcurr[pos(i, j - 1)]) / dt * (1. + omk * Cucurr[pos(i, j - 1)]) + dphidt * (1. + omk * u)) * dpyd / dnormd;
        ...
        if ((1. - phi) >= 1.e-6)
        {
            DivompGradU = 0.5 * ((2. - F[pos(i + 1, j)] - phi) * (Cucurr[pos(i + 1, j)] - u) - (2. - F[pos(i - 1, j)] - phi) * (u - Cucurr[pos(i - 1, j)]) + (2. - F[pos(i, j + 1)] - phi) * (Cucurr[pos(i, j + 1)] - u) - (2. - F[pos(i, j - 1)] - phi) * (u - Cucurr[pos(i, j - 1)])) / dx / dx;
        }
        else
        {
            DivompGradU = 0.;
...
void WriteFields(char Prefix[LENMAX], int index, REAL *P, REAL *F, REAL *Cu, int *Pha, Constants *Param, int SVG)
{
    ...
    // Name Output Files
    //================================================
    char FileNameP[LENMAX];
    char FileNameC[LENMAX];
    char FileNameCompoX[LENMAX];
    char Indx[LENMAX];

    // SVG = 0 output phi and Supersaturation
    char PField[LENMAX] = "PF";
    char CField[LENMAX] = "C";

// Phase & Grain output if necessary
#if (OUTPUT_Pha)
    char FileNamePha[LENMAX];
    char PhaField[LENMAX] = "Phase";
#endif
#if (OUTPUT_Grains)
    char FileNameGra[LENMAX];
    char GraField[LENMAX] = "Grain";
#endif

    // SVG>0 then output Psi and Composition
    if (SVG > 0)
    {
        sprintf(PField, "Psi");
    ...
    sprintf(FileNameP, "%s_%s.%s.vtk", PField, Prefix, Indx);
    sprintf(FileNameC, "%s_%s.%s.vtk", CField, Prefix, Indx);
    sprintf(FileNameCompoX, "CompoX_%s.%s.dat", Prefix, Indx);

#if (OUTPUT_Pha)
    sprintf(FileNamePha, "%s_%s.%s.vtk", PhaField, Prefix, Indx);
#endif
#if (OUTPUT_Grains)
    sprintf(FileNameGra, "%s_%s.%s.vtk", GraField, Prefix, Indx);
#endif
    ...
    REAL grain;
#endif
    // For phi and supersaturation, XmaxC and F is Nx
    int XmaxC = Nx;
    int XmaxP = Nx;

#if (OUTPUT_Pha || OUTPUT_Grains)
    int XmaxG = Nx + 2;
#endif
    ...
    fprintf(OutFileP, "# vtk DataFile Version 3.0\n");
    fprintf(OutFileP, "Delta %g\n", DeltaF);
    fprintf(OutFileP, "ASCII\n");
    fprintf(OutFileP, "DATASET STRUCTURED_POINTS\n");
    fprintf(OutFileP, "DIMENSIONS %d %d %d\n", (XmaxP - 0), Ny + 2, 1);
    fprintf(OutFileP, "SPACING %f %f %f\n", 1., 1., 1.);
    fprintf(OutFileP, "ORIGIN 0 0 0\n");
    fprintf(OutFileP, "POINT_DATA %d\n", (XmaxP - 0) * (Ny + 2));
    fprintf(OutFileP, "SCALARS PF double 1\n");
    fprintf(OutFileP, "LOOKUP_TABLE default\n");
    ...
    fprintf(OutFileC, "SPACING %f %f %f\n", 1., 1., 1.);
...
#if (OUTPUT_Pha)
    // .vtk header for Phase
    FILE *OutFilePha;
    OutFilePha = fopen(FileNamePha, "w");
    fprintf(OutFilePha, "# vtk DataFile Version 3.0\n");
    fprintf(OutFilePha, "Delta %g\n", DeltaF);
    fprintf(OutFilePha, "ASCII\n");
    fprintf(OutFilePha, "DATASET STRUCTURED_POINTS\n");
    fprintf(OutFilePha, "DIMENSIONS %d %d %d\n", (XmaxP - 0), Ny + 2, 1);
    fprintf(OutFilePha, "SPACING %f %f %f\n", 1., 1., 1.);
    fprintf(OutFilePha, "ORIGIN 0 0 0\n");
    fprintf(OutFilePha, "POINT_DATA %d\n", (XmaxP - 0) * (Ny + 2));
    fprintf(OutFilePha, "SCALARS Phase int 1\n");
    fprintf(OutFilePha, "LOOKUP_TABLE default\n");
#endif

#if (OUTPUT_Grains)
    // .vtk header for Grains
    FILE *OutFileGra;
    OutFileGra = fopen(FileNameGra, "w");
    fprintf(OutFileGra, "# vtk DataFile Version 3.0\n");
    fprintf(OutFileGra, "Delta %g\n", DeltaF);
    fprintf(OutFileGra, "ASCII\n");
    fprintf(OutFileGra, "DATASET STRUCTURED_POINTS\n");
    fprintf(OutFileGra, "DIMENSIONS %d %d %d\n", (XmaxG - 0), Ny + 2, 1);
    fprintf(OutFileGra, "SPACING %f %f %f\n", 1., 1., 1.);
    fprintf(OutFileGra, "ORIGIN 0 0 0\n");
    fprintf(OutFileGra, "POINT_DATA %d\n", (XmaxG - 0) * (Ny + 2));
    fprintf(OutFileGra, "SCALARS Grain double 1\n");
    fprintf(OutFileGra, "LOOKUP_TABLE default\n");
#endif
    ...
    if (SVG > 0)
    {
        for (int j = 0; j < Ny + 2; j++)
        {
            for (int i = 0; i < XmaxP; i++)
            {
                psi = P[pos(i, j)];
                c = 0.5 * (opk - omk * tanh(psi / sqrt2)) * (1. + omk * Cu[pos(i, j)]);
                fprintf(OutFileP, DECIMALS_SVG, psi);
    ...
    else
    {
        for (int j = 0; j < Ny + 2; j++)
        {
            for (int i = 0; i < XmaxP; i++)
            {
                phi = tanh(P[pos(i, j)] / sqrt2);
                ;
                fprintf(OutFileP, DECIMALS_P, phi);
            }
        }

        for (int j = 0; j < Ny + 2; j++)
        {
            for (int i = 0; i < XmaxC; i++)
            {
                c = 0.5 * (opk - omk * tanh(P[pos(i, j)] / sqrt2)) * (1. + omk * Cu[pos(i, j)]);
...
// Phase
    ...
    for (int j = 0; j < Ny + 2; j++)
    {
        for (int i = 0; i < XmaxG; i++)
        {
            phase = Pha[pos(i, j)];
            fprintf(OutFilePha, "%d ", phase);
        }
    }
    fclose(OutFilePha);
#endif
// Grain
    ...
    for (int j = 0; j < Ny + 2; j++)
    {
        for (int i = 0; i < XmaxG; i++)
        {
            grain = (float)(Pha[pos(i, j)] * (1. + tanh(P[pos(i, j)] / sqrt2)) / 2.);
            ;
            fprintf(OutFileGra, "%d ", grain);
        }
    }
    fclose(OutFileGra);
#endif
    ...
    // Composition profile at the tip location
    ...
    fprintf(OutFileCompoX, "#itip=%g \t", xmax);
    fprintf(OutFileCompoX, "jtip=%d \t", advjtip);
    fprintf(OutFileCompoX, "x \t");
    fprintf(OutFileCompoX, "c/cl0 \n");

    for (int i = 0; i < Nx + 2; i++)
    {
        phi = tanh(P[pos(i, advjtip)] / sqrt2);
        c = 0.5 * (opk - omk * phi) * (1. + omk * Cu[pos(i, advjtip)]);
