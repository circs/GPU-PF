...
//    SIMULATION ID
// -----------------------
#define PREFIX "Ny128" // Prefix of Output File Name
#define Ny 126         // Dimension Y
#define Nz 126         // Dimension Z
#define INIT_FILE "Ny60_int_D1500_G12_k03_V60_dx12_W500"
...
#define GRAD0 (12. * 100) // K/m
#define GRAD1 (12. * 100) // K/m
#define TIME0 0           // h
#define TIME1 0           // h
// changed
#define TIMELH 0 // s
...
#define GIBBSTHOMSON (6.478e-08) // K.m
...
// -----------------------
// Dimensions
#define TOTALTIME (165.407616) // [seconds]
#define Nx 1534          // Dimension X
...
//#define POSITION_0 (800. / W_microns) // [W]

// -----------------------
// Off sets PRISMS-PF like
// -----------------------
#define U_off  0.0
#define x00 0.


#define POSITION_0 379.28 // [W]
...
#define IQY 6  // Wavenumber of the initial perturbation /Y
#define IQZ 6  // Wavenumber of the initial perturbation /Z
#define AMP (12.1905/dx) // [dx]
...
//      OUTPUT
// -----------------------
#define NOUTFIELDS 57
#define NOUTTIP 400
#define NOUTSVG 1
#define COMPRESS 1
#define OUTINDEXSEC 0
#define SUMMARIZE 1
...
int main(int argc, char **argv) {
    ...
    int IterOutFields_Raw = niter;
    #if (NOUTFIELDS > 0)
        IterOutFields = niter / NOUTFIELDS;
        IterOutFields_Raw = niter / NOUTFIELDS;
        ...
        #endif
                {
                    ...
                    ////////////////////////
                        ...
                        if (iter % IterOutFields_Raw == 0){
                            ...
                            int indx = iter / IterOutFields_Raw;
                            #if (OUTINDEXSEC)
...
__global__ void Init(REAL *P1, REAL *P2, REAL *U1, REAL *U2, Constants *Param, curandState *state) {
    ...
    REAL xintyz = xint / dx;
        ...
        #else
            //----------------------------------------------------------------
            // Sine perturbation of amplitude AMP and wave numbers IQY and IQZ
            //----------------------------------------------------------------
            #if (BOUND_COND_Z == ANTISYM)
                xintyz -= AMP * (1. - cos(IQY * 3.14159 * j / Ny / 2.) * cos(IQZ * 3.14159 * k / Nz / 2.));
            #else
                //xintyz -= AMP * (1. + cos(IQY * 3.14159 * j / Ny) * cos(IQZ * 3.14159 * k / Nz));
                xintyz += 0.25 * AMP * (2. + cos(IQY * 3.14159 * (1. - ((float)j - 0.5) / Ny)) + cos(IQZ * 3.14159 * (1. - ((float)k - 0.5) / Nz)));
            #endif
        #endif
    #endif
        ...
        #endif
        {
            ...
            ///////
                ...
                // no thermal effect
                        ...
                        //Changed for PRISMS-PF, xoffs = 0 for non-treadmill
                        //REAL temp = (i * dx - x00 - Vp * dt * iter) / lT + U_off;
            ...
            //Changed for PRISMS-PF
            REAL Tau = (Cucurr[pos(i, j, k)] > 1.) ? kcoeff : (1. + omk * Cucurr[pos(i, j, k)]);
